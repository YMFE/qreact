    ...
    describe('defaultProps', () => {
        ...
        it('should cache default props', () => {
            ...
            class WithDefaultProps extends Component {
                constructor(props, context) {
                    super(props, context);
                    expect(this.props).to.be.deep.equal(cmpProps);
                }
                render() {
                    return <div />;
                }
            }
            WithDefaultProps.defaultProps = { fieldA: 1, fieldC: 1 };
            render((
                <div>
            ...
            let getDefaultProps = sinon.spy(() => {
                return WithDefaultProps.defaultProps;
            })
            let NewWithDefaultProps = createClass({
                getDefaultProps,
                getInitialState() {
                    expect(this.props).to.be.deep.equal(cmpProps);
                    return {}
            ...
            render((
                <div>
                    <NewWithDefaultProps fieldB={2} fieldD={2} fieldX={10} />
                    <NewWithDefaultProps fieldB={2} fieldD={2} fieldX={10} />
                    <NewWithDefaultProps fieldB={2} fieldD={2} fieldX={10} />
                </div>
            ), scratch);
            expect(getDefaultProps).to.be.calledOnce;
        });
    });
});

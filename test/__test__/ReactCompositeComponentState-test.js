...
describe("ReactCompositeComponent-state", () => {
  ...
  it("should support setting state", () => {
    ...
    //var stateListener = sinon.spy();
    var instance = React.render(
      <TestComponent stateListener={wrap.stateListener} ref={(c)=>{c.peekAtState("initial-callback");}} />,
      container
    );
    React.render(
      <TestComponent stateListener={wrap.stateListener} nextColor="green" />,
      container,
    ...
    mock.expects("stateListener").withExactArgs(expected.join("\n"));
  ...
  it("should merge state when sCU returns false", function() {
    ...
    const test = React.render(<Test />, container);
    test.setState({b: 0});
    console.log(log);

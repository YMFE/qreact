(function(global, factory) {
  ...
  function diffVchildren(patches, vnode, newVnode, node, parentContext) {
    if (!node.vchildren) {
      return;
    } // react-lite hasn't seen this DOM node before
  ...
  function setPropValue(node, name, value) {
    ...
    if (propInfo) {
      ...
      VALID_ATTRIBUTE_NAME_REGEX.test(name)
    ) {
      if (value == null) {
    ...
    if (argsCache) {
      if (argsCache === true) {
        ...
        argsCache.callback = argsCache.callback
          ? pipe(argsCache.callback, callback)
          : callback;
      }
      return;
    }
  ...
  function cloneElement(originElem, props) {
    ...
    var vnode = createElement.apply(
      undefined,
      [type, newProps].concat(children)
    );

/**
 * 此版本带有selection by 司徒正美 Copyright 2018-03-26
 * IE9+
 */
...
(function(global, factory) {
  ...
  var msie =
    document$1.documentMode ||
    versions[typeNumber(document$1.all) + "" + typeNumber(win.XMLHttpRequest)];
  ...
  function createElement$1(vnode, p) {
    ...
    switch (type) {
      ...
      case "#comment":
        return document$1.createComment(text);
      case "svg":
        ns = NAMESPACE.svg;
        break;
      case "math":
  ...
  function insertElement(fiber, mountPoint) {
    ...
    var prevFocus = isElement && document$1.activeElement;
    parentNode.insertBefore(dom, after);
    if (
      isElement &&
      prevFocus !== document$1.activeElement &&
      contains(document$1.body, prevFocus)
    ) {
      try {
  ...
  function dispatchEvent(e, type, end) {
    ...
    document$1.__async = true;
    triggerEventFlow(paths, captured, e);
    if (!e._stopPropagation) {
      triggerEventFlow(paths.reverse(), bubble, e);
    }
    document$1.__async = false;
    flushUpdaters();
  ...
  var fixWheelType =
    document$1.onwheel !== void 666
      ? "wheel"
      : "onmousewheel" in document$1 ? "mousewheel" : "DOMMouseScroll";
  ...
  function createContext(defaultValue, calculateChangedBits) {
    ...
    Provider.childContextTypes = _defineProperty(
      {},
      contextProp,
  ...
  function pushError(instance, hook, error) {
    ...
    if (catchUpdater) {
      ...
      catchUpdater.errorInfo = catchUpdater.errorInfo || [
        error,
  ...
  ComponentFiber.prototype = {
    ...
    hydrate: function hydrate(updateQueue, inner) {
      ...
      if (
        !this._forceUpdate &&
        !captureError(instance, "shouldComponentUpdate", [
          props,
          state,
          context
        ])
  ...
  function renderByAnu(vnode, root, callback) {
    ...
    if (rootIndex !== -1) {
      ...
      if (wrapperFiber._hydrating) {
        wrapperFiber._pendingCallbacks.push(
          renderByAnu.bind(null, vnode, root, callback, context)
        );
        return wrapperFiber.child.stateNode;
      }
      wrapperFiber = receiveVnode(
        wrapperFiber,
        wrapperVnode,
        updateQueue,
        mountCarrier
    ...
    if (callback) {
      wrapperFiber._pendingCallbacks.push(
        callback.bind(wrapperFiber.child.stateNode)
      );
    }
  ...
  function diffChildren(
    ...
    if (isEmpty) {
      ...
      for (var _i2 in children) {
        vnode = children[_i2];
        fiber = children[_i2] = matchFibers[_i2]
          ? receiveVnode(matchFibers[_i2], vnode, updateQueue, mountCarrier)
  ...
  var useIEOffsets =
    inBrowser && "selection" in document && !("getSelection" in win);

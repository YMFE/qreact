/**
 * Qreact要求浏览器支持Map对象，没有createClass, createFactory, PropTypes, isValidElement,
 ...
 */
  ...
  try {
    var w = window;
    var b = !!w.alert; // eslint-disable-line
  } catch (e) {
    b = false;
    w = {
      document: fakeDoc
  ...
  function emptyElement(node) {
    var child;
    while (child = node.firstChild) { // eslint-disable-line
      if (child.nodeType === 1) {
        emptyElement(child);
      }
  ...
  var modern = /NaN|undefined/.test(msie) || msie > 8; // eslint-disable-line
 ...
 */
  ...
  function addEvent(el, type, fn, bool) {
    ...
    } else if (el.attachEvent) { // eslint-disable-line

    ...
    config.plugins.forEach(function(plugin) {
        if (plugin.config) {
            ...
            if (pluginRedux) {
                ...
                configRedux.initialState = merge(
                    config.redux.initialState,
                    pluginRedux.initialState
                );
                configRedux.reducers = merge(
                    config.redux.reducers,
                    pluginRedux.reducers
                );
                configRedux.rootReducers = merge(
                    config.redux.rootReducers,
                    pluginRedux.reducers
                );
                configRedux.enhancers = configRedux.enhancers.concat(
                    pluginRedux.enhancers || []
                );
                configRedux.middlewares = configRedux.middlewares.concat(
                    pluginRedux.middlewares || []
                );
                configRedux.combineReducers =
                    configRedux.combineReducers || pluginRedux.combineReducers;
                configRedux.createStore =
                    configRedux.createStore || pluginRedux.createStore;
            }
        }
    });
    return config;

...
describe("ReactJSXElementValidator", () => {
    ...
    it("warns for keys for arrays of elements with owner info", () => {
        ...
        class ComponentWrapper extends React.Component {
            render() {
                return (
                    <InnerComponent childSet={[<Component />, <Component />]} />
                );
            }
        }
            ...
            "@@iterator": function() {
                ...
                return {
                    next: function() {
                        const done = ++i > 2;
                        return {
                            value: done ? undefined : <Component />,
                            done: done
                        };
                    }
                };
            ...
            "@@iterator": function() {
                ...
                return {
                    next: function() {
                        const done = ++i > 2;
                        return {
                            value: done ? undefined : [i, <Component />],
                            done: done
                        };
                    }
                };
    ...
    it("should not check the default for explicit null", () => {
        expect(() =>
            ReactTestUtils.renderIntoDocument(
                <RequiredPropComponent prop={null} />
            )
        ).toWarnDev(
            "Warning: Failed prop type: The prop `prop` is marked as required in " +
                "`RequiredPropComponent`, but its value is `null`.\n" +
    ...
    it("should check declared prop types", () => {
        ...
        ReactTestUtils.renderIntoDocument(
            <RequiredPropComponent prop="string" />
        );
    });

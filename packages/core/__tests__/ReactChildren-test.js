...
describe("ReactChildren", () => {
    ...
    it("should not enumerate enumerable numbers (#4776)", () => {
        ...
        try {
            ...
            var callback = jasmine
                .createSpy()
                .and.callFake(function(kid, index) {
                    expect(this).toBe(context);
                    return kid;
                });
    ...
    it("should flatten children to an array", () => {
        ...
        expect(
            React.Children.toArray([1, "two", null, undefined, true])
        ).toEqual([1, "two"]);
    });
    ...
    it("复杂的孩子转换", function() {
        function getString(nodes) {
            ...
            for (var i = 0, node; (node = nodes[i++]); ) {
                if (
                    node.nodeType === 8 &&
                    node.nodeValue.indexOf("react-text") !== 0
                ) {
                    continue;
                }
        ...
        expect(getString(div.firstChild.childNodes)).toBe(
            "#text p span strong"
        );
        s.change(100);
        expect(getString(div.firstChild.childNodes)).toBe(
            "em span #text span b i"
        );
        s.change(100);
